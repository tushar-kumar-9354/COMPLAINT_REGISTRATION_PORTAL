...
from utils.genai import get_tone, get_priority, generate_auto_response
...
def complaint_form(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # 🔥 Generate AI fields before saving
            complaint.tone = get_tone(complaint.description)
            complaint.priority = get_priority(complaint.description)
            complaint.auto_response = generate_auto_response(complaint.description)
            # Optional: implement and call generate_summary if needed
            # complaint.ai_summary = generate_summary(complaint.description)
...
def filter_complaints(request):
    ...
    if category_filter:
        complaints = complaints.filter(category__iexact=category_filter)
